...
class App:
    ...
    def create_widgets(self, vcmd):
        tk.Label(self.master, text="欢迎使用战地5公告机！如有问题请加QQ群：619982307", anchor="w", padx=15).pack(fill=tk.X)

        self.entry_range_time = self.create_entry("发送次数：", "10", vcmd)
        self.entry_range_rest = self.create_entry("间隔秒数：", "120", vcmd)

        tk.Label(self.master, text="注意：由于战地5限制，字符数限制80，必须全部为小写字母，不得使用中文字符", anchor="w", padx=15).pack(fill=tk.X)

        self.entry_welcome = self.create_entry("欢迎内容：", self.messages["welcome"], vcmd)
        ...
        tk.Label(self.master, text="注意：输入法需切换为英语，不能使用微软拼音", anchor="w", padx=15).pack(fill=tk.X)
        
        self.start_button = tk.Button(self.master, text="开始发送", command=self.start_sending)
        self.start_button.pack(side=tk.LEFT, padx=10, pady=10)

        self.stop_button = tk.Button(self.master, text="停止发送", command=self.stop_sending, state=tk.DISABLED)
    ...
    def start_sending(self):
        ...
        threading.Thread(target=send_keyboard_messages, args=(range_time, range_rest, messages)).start()
